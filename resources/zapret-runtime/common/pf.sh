...
pf_anchor_root()
{
	local patch
	[ -f "$PF_MAIN" ] && {
		# --- Стандартный путь: вставляем якоря рядом с apple-якорями ---
		grep -q '^rdr-anchor "zapret"$' "$PF_MAIN" || {
			grep -q '^rdr-anchor "com\.apple/\*"$' "$PF_MAIN" && {
...
' $PF_MAIN
		...
		grep -q '^anchor "zapret"$' "$PF_MAIN" || {
			grep -q '^anchor "com\.apple/\*"$' "$PF_MAIN" && {
...
' $PF_MAIN
		...
		grep -q "^set limit table-entries" "$PF_MAIN" || {
			grep -q '^scrub-anchor "com\.apple/\*"$' "$PF_MAIN" && {
...
' $PF_MAIN
		...
		# --- Фолбэк для нестандартного pf.conf (например, переписанного VPN) ---
		# apple-якорей нет, поэтому вставлять не к чему. Кладём якоря так, чтобы
		# не сломать порядок PF: rdr-anchor (трансляция) и set limit — в начало,
		# anchor (фильтрация) — в конец файла.
		grep -q '^rdr-anchor "zapret"$' "$PF_MAIN" || {
			echo "apple-якоря не найдены — добавляю rdr-anchor \"zapret\" в начало $PF_MAIN (режим совместимости с VPN)"
			patch=1
			printf 'rdr-anchor "zapret"\n%s\n' "$(cat "$PF_MAIN")" >"$PF_MAIN.zapret.tmp" &&
				cat "$PF_MAIN.zapret.tmp" >"$PF_MAIN"
			rm -f "$PF_MAIN.zapret.tmp"
		}
		grep -q "^set limit table-entries" "$PF_MAIN" || {
			echo "добавляю set limit table-entries в начало $PF_MAIN"
			patch=1
			printf 'set limit table-entries 5000000\n%s\n' "$(cat "$PF_MAIN")" >"$PF_MAIN.zapret.tmp" &&
				cat "$PF_MAIN.zapret.tmp" >"$PF_MAIN"
			rm -f "$PF_MAIN.zapret.tmp"
		}
		grep -q '^anchor "zapret"$' "$PF_MAIN" || {
			echo "добавляю anchor \"zapret\" в конец $PF_MAIN"
			patch=1
			printf '\nanchor "zapret"\n' >>"$PF_MAIN"
		}
